...
class GeoAnalysis:
    ...
    def hd(self, pm, pi, hsi):
        size = min(hsi.shape, pm.shape, pi.shape)
        output = hsi[:size[0], :size[1]] + pi[:size[0], :size[1]] - pm[:size[0], :size[1]]
        return output

    def hsf(self, pm, cc, etr, hd):
        size = min(hd.shape, pm.shape, etr.shape, cc.shape)
        output = np.empty(size)
        for i in range(size[0]):
            output[i] = np.minimum(hd[i][:size[1]] + pm[i][:size[1]] - etr[i][:size[1]], cc[i][:size[1]])
        return output

    def rp(self, pi, etr, hsi, hsf):
        size = min(pi.shape, hsi.shape, hsf.shape, etr.shape)
        output = pi[:size[0], :size[1]] + hsi[:size[0], :size[1]] - hsf[:size[0], :size[1]] - etr[:size[0], :size[1]]
        return output
...
class ArrayTools:
    ...
    def data_fixer(self, n_std):
        ...
        self.array[self.array > mean + n_std * std] = mean
        self.array[self.array < mean - n_std * std] = mean

    def array_avg(self):
        valuescount = self.array[~np.isnan(self.array)].size
        values_sum = np.nansum(self.array)
...
for i in range(len(months)):
    ...
    ArrayTools(hd_array).data_fixer(n_std=3)
    ...
    ArrayTools(rp_array).data_fixer(n_std=3)
    if i == 0:
        rp_annual = rp_array
    else:
        rp_annual += rp_array
